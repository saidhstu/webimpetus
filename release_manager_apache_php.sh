...
docker build -f devops/kubernetes/Dockerfile --build-arg TAG=${TARGET_STACK}-${DATE_GEN_VERSION}  -t wsl-${TARGET_STACK} .
...
helm upgrade --install -f devops/webimpetus-chart/values-${targetNs}.yaml wsl-${targetNs} ./devops/webimpetus-chart --set-string targetImage="wsl-${TARGET_STACK}" --set-string targetImageTag="${TARGET_STACK}-${DATE_GEN_VERSION}" --namespace ${targetNs}
else
