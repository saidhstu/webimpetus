...
helm upgrade --install -f devops/webimpetus-chart/values-${targetNs}.yaml wsl-${targetNs} ./devops/webimpetus-chart --set-string targetImage="registry.workstation.co.uk/wsl-${TARGET_STACK}:${DATE_GEN_VERSION}" --set-string targetImageTag="${DATE_GEN_VERSION}" --namespace ${targetNs}
else
