...
helm delete -f devops/kubernetes/wsl-deployment.yaml
fi
...
helm upgrade --install workstation-${targetEnv} devops/webimpetus-chart -f devops/webimpetus-chart/values-${targetEnv}.yaml --namespace ${targetEnv} --create-namespace
fi
